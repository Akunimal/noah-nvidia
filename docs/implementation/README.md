...
The onboarding phases 0, 1, 2 and 3 are closed locally. The contract keeps
...
draft; the phase 3 endpoint validates strict JSON and never writes. Only a
later explicit completion can apply it. Skip shows the synthetic-data warning
but does not seed anything until phase 4.
